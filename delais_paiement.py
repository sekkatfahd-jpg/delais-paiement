...
def traiter_rapprochement(grand_livre_df, dict_fournisseurs, journaux_achat, journaux_banque):
        ...
        for fac_info in factures_avec_solde:
            ...
            # Si la facture n'a que des paiements (pas d'avoir)
                ...
                # Plusieurs paiements sans écart de change spécial
                    ...
                    # CORRECTION BUG 1: Toujours utiliser 'montant' (partiel) et non 'total_paiement'
                    montant_paiement = paiement_aff['montant']

                    # Calculer le solde brut (avant OD)
                    solde_brut = montant_original - montant_paiement
                    
                    # L'OD est réparti au prorata pour que le solde soit 0 si la facture est payée
                    # ou pour refléter l'écart réel si partiellement payée
                    if abs(solde_brut) < 0.01:
                        # Facture entièrement payée - solde = 0
                        montant_facture_affiche = montant_paiement
                        od_affiche = 0
                        solde = 0
                    else:
                        # Facture partiellement payée ou avec écart
                        montant_facture_affiche = montant_original
                        od_affiche = montant_od_signe
                        solde = solde_brut + od_affiche
    ...
    # CORRECTION BUGS 2/3: Identifier les lettrages qui ont des paiements mais PAS de factures 'ac'
    # Ces paiements ne doivent PAS être réaffectés ailleurs - ils appartiennent à des groupes
    # avec factures 'AD' qui sont correctement lettrés dans le GL
    lettrages_sans_factures_ac = set()
    for key in paiements_par_groupe:
        compte, lettrage = key
        if lettrage:  # Lettrage non vide
            # Vérifier si ce groupe a des factures 'ac'
            if key not in factures_par_groupe or len(factures_par_groupe[key]) == 0:
                lettrages_sans_factures_ac.add(key)
    
    # Identifier les paiements non lettrés (tous les mouvements != 0)
    # MAIS exclure les paiements qui appartiennent à des lettrages sans factures 'ac'
    paiements_non_lettres_raw = paiements[
        (paiements['LettrageCorrige'] == '') &
        (paiements['MontantMvt'] != 0)
    ].copy()
    
    # Filtrer pour exclure les paiements des lettrages sans factures 'ac'
    # Ces paiements ont un Lettrage original qui est dans paiements_par_groupe mais pas dans factures_par_groupe
    def paiement_est_dans_groupe_sans_factures(row):
        compte = str(row['Compte']).strip()
        lettrage = str(row['Lettrage']).strip() if pd.notna(row['Lettrage']) else ''
        if lettrage:
            key = (compte, lettrage)
            # Si ce lettrage est dans paiements_par_groupe mais pas dans factures_par_groupe
            if key in lettrages_sans_factures_ac:
                return True
        return False
